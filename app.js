...
hbs.registerHelper('dateFormatLong', function (date, options) {
  const formatToUse = (arguments[1] && arguments[1].hash && arguments[1].hash.format) || "h:mm:ss a - MM/DD/YYYY"
  return moment(date).format(formatToUse);
});
hbs.registerHelper('dateCalendar', function (date, options) {
  const formatToUse = (arguments[1] && arguments[1].hash && arguments[1].hash.format) || "calendar()"
  return moment(date).format(formatToUse);
});
// view engine setup
